-- Keep vector retrieval as the primary signal, while adding a small full-text
-- boost for exact terms in short shopper queries (for example, "classic bag").
-- This avoids depending on a generative query rewrite for lexical intent.
create index if not exists products_search_document_fts_idx
